...
StartCoroutine(GameLoop()); //Iniciar juego
    ...
    private string EndMessage()
    {
        ...
        if (m_GameWinner != null)
        {
            PlayerPrefs.SetString("ganador", m_GameWinner.m_ColoredPlayerText);
            message = m_GameWinner.m_ColoredPlayerText + " GANA EL JUEGO!";
            StartCoroutine(CargarEscena());

            Debug.Log("COanas   " + numeroGanador);

            if (m_GameWinner.m_ColoredPlayerText == "PLAYER 1")
            {
                PlayerPrefs.SetInt("Juga",1);
            }


            if (m_GameWinner.m_ColoredPlayerText == "PLAYER 2")
            {
                PlayerPrefs.SetInt("Juga", 2);
            }
